...
provider "yandex" {
  # Аутентификация через ключ сервисного аккаунта (рекомендуемый способ)
  # Путь к файлу раскрывается функцией pathexpand() для поддержки ~
  service_account_key_file = pathexpand(var.service_account_key_file)

  # Идентификаторы облака и каталога для размещения ресурсов
